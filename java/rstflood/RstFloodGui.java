...
/**
 * This GUI class provides user interface to setup RST flood DoS attack
 *
 * @author Gity a.s.
 */

public class RstFloodGui extends AbstractSamplerGui{
	@Serial
    ...
    public String getLabelResource() {
        return DDOS_RST_FLOOD_TITLE; // $NON-NLS-1$
    }
    ...
    public String getStaticLabel() {
        return JMeterUtils.getLocaleString(DDOS_RST_FLOOD_TITLE);
    }
    ...
    public TestElement createTestElement() {
        RstFloodSampler sampler = new RstFloodSampler();
    ...
    public void modifyTestElement(TestElement te) {
        ...
        if (selectedDev != null) { //Maybe no network devices were found       
            te.setProperty(RstFloodSampler.INTERF, selectedDev.toString());
        }

        te.setProperty(RstFloodSampler.DMAC, dMAC.getText());
        te.setProperty(RstFloodSampler.SMACSINGLE, sMACsingle.getText());
        te.setProperty(RstFloodSampler.SMACMIN, sMACmin.getText());
        te.setProperty(RstFloodSampler.SMACMAX, sMACmax.getText());
        te.setProperty(RstFloodSampler.RANDOMMAC, incrementalMAC.isSelected());
        te.setProperty(RstFloodSampler.TARGETIP, targetIP.getText());
        te.setProperty(RstFloodSampler.TARGETIPV6, targetIPv6.getText());
        te.setProperty(RstFloodSampler.SOURCEIPSINGLE, sourceIPsingle.getText());
        te.setProperty(RstFloodSampler.SOURCEIPV6, sourceIPv6.getText());
        te.setProperty(RstFloodSampler.RANDOMIP, randomIP.isSelected());
        te.setProperty(RstFloodSampler.RANDOMIPV6, randomIPv6.isSelected());
        te.setProperty(RstFloodSampler.IPV6ENABLED, ipv6Enabled.isSelected());
        te.setProperty(RstFloodSampler.NUMBEROFIPV6ADDRESSES, numberOfIPv6Addresses.getText());
        te.setProperty(RstFloodSampler.SOURCEIPMIN, sourceIPMIN.getText());
        te.setProperty(RstFloodSampler.SOURCEIPMAX, sourceIPMAX.getText());
        te.setProperty(RstFloodSampler.SOURCEIPV6MIN, sourceIPv6Min.getText());
        te.setProperty(RstFloodSampler.SOURCEIPV6MAX, sourceIPv6Max.getText());
        te.setProperty(RstFloodSampler.TTL, ttl.getText());
        te.setProperty(RstFloodSampler.SOURCEPORTSINGLE, sourcePortSingle.getText());
        te.setProperty(RstFloodSampler.RANDOMPORT, randomPort.isSelected());
        te.setProperty(RstFloodSampler.SOURCEPORTMIN, sourcePortMIN.getText());
        te.setProperty(RstFloodSampler.SOURCEPORTMAX, sourcePortMAX.getText());
        te.setProperty(RstFloodSampler.DPORT, destPort.getText());
        te.setProperty(RstFloodSampler.WINSIZE, windowSize.getText());
        te.setProperty(RstFloodSampler.PAYLOAD, payLoad.getText());
        te.setProperty(RstFloodSampler.NUMBER, number.getText());
        te.setProperty(RstFloodSampler.RATE, rate.getText());

        //Following blocks assigns final values for trafgen config to separate properties 
        if (incrementalMAC.isSelected()) {
            te.setProperty(RstFloodSampler.SMACTG, DDoSUtils.getInstance().macIncremental(sMACmin.getText(), sMACmax.getText()));
        } else {
            te.setProperty(RstFloodSampler.SMACTG, sMACsingle.getText());
        }
        if (randomIP.isSelected()) {
            te.setProperty(RstFloodSampler.SOURCEIPTG, DDoSUtils.getInstance().ipRandom(sourceIPMIN.getText(), sourceIPMAX.getText()));
        } else {
            te.setProperty(RstFloodSampler.SOURCEIPTG, sourceIPsingle.getText());
        }
        if (randomPort.isSelected()) {
            te.setProperty(RstFloodSampler.SOURCEPORTTG, "drnd(" + sourcePortMIN.getText() + "," + sourcePortMAX.getText() + ")");
        } else {
            te.setProperty(RstFloodSampler.SOURCEPORTTG, sourcePortSingle.getText());
        }
        ((RstFloodSampler) te).checkInit();
    }
    ...
    public void configure(TestElement element) {
        ...
        selectInt.setSelectedItem(element.getPropertyAsString(RstFloodSampler.INTERF));

        dMAC.setText(element.getPropertyAsString(RstFloodSampler.DMAC));
        sMACsingle.setText(element.getPropertyAsString(RstFloodSampler.SMACSINGLE));
        incrementalMAC.setSelected(element.getPropertyAsBoolean(RstFloodSampler.RANDOMMAC, false));
        sMACmin.setText(element.getPropertyAsString(RstFloodSampler.SMACMIN));
        sMACmax.setText(element.getPropertyAsString(RstFloodSampler.SMACMAX));
        targetIP.setText(element.getPropertyAsString(RstFloodSampler.TARGETIP));
        targetIPv6.setText(element.getPropertyAsString(RstFloodSampler.TARGETIPV6));
        sourceIPsingle.setText(element.getPropertyAsString(RstFloodSampler.SOURCEIPSINGLE));
        sourceIPv6.setText(element.getPropertyAsString(RstFloodSampler.SOURCEIPV6));
        randomIP.setSelected(element.getPropertyAsBoolean(RstFloodSampler.RANDOMIP, false));
        randomIPv6.setSelected(element.getPropertyAsBoolean(RstFloodSampler.RANDOMIPV6));
        ipv6Enabled.setSelected(element.getPropertyAsBoolean(RstFloodSampler.IPV6ENABLED));
        numberOfIPv6Addresses.setText(element.getPropertyAsString(RstFloodSampler.NUMBEROFIPV6ADDRESSES));
        sourceIPMIN.setText(element.getPropertyAsString(RstFloodSampler.SOURCEIPMIN));
        sourceIPMAX.setText(element.getPropertyAsString(RstFloodSampler.SOURCEIPMAX));
        sourceIPv6Min.setText(element.getPropertyAsString(RstFloodSampler.SOURCEIPV6MIN));
        sourceIPv6Max.setText(element.getPropertyAsString(RstFloodSampler.SOURCEIPV6MAX));

        ttl.setText(element.getPropertyAsString(RstFloodSampler.TTL));
        destPort.setText(element.getPropertyAsString(RstFloodSampler.DPORT));
        sourcePortSingle.setText(element.getPropertyAsString(RstFloodSampler.SOURCEPORTSINGLE));
        randomPort.setSelected(element.getPropertyAsBoolean(RstFloodSampler.RANDOMPORT, false));
        sourcePortMIN.setText(element.getPropertyAsString(RstFloodSampler.SOURCEPORTMIN));
        sourcePortMAX.setText(element.getPropertyAsString(RstFloodSampler.SOURCEPORTMAX));
        windowSize.setText(element.getPropertyAsString(RstFloodSampler.WINSIZE));
        payLoad.setText(element.getPropertyAsString(RstFloodSampler.PAYLOAD));

        ((RstFloodSampler) element).checkInit();
        if (((RstFloodSampler) element).isDynamicRate()) {
            dynamicRate = true;
            element.setProperty(RstFloodSampler.DYNAMICRATE, true); //Must be here to prevent DYNAMICRATE property from being deleted by JMeter
            number.setEnabled(false);
            rate.setEnabled(false);
        } else {
            dynamicRate = false;
            element.setProperty(RstFloodSampler.DYNAMICRATE, false); //Must be here to prevent DYNAMICRATE property from being deleted by JMeter
            number.setText(element.getPropertyAsString(RstFloodSampler.NUMBER));
            rate.setText(element.getPropertyAsString(RstFloodSampler.RATE));
